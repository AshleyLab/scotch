...
# get mean depth across entire matrix
def get_mean_depth() -> float:
	# TODO
	mean: float = 0.0
	assert False
	assert mean != 0.0 #keep this here; we can't proceed is mean = 0
	return mean
matrix_mean_depth = get_mean_depth()

# get mean nReads across entire matrix
def get_mean_nReads() -> float: 
	# TODO
	mean: float = 0.0
	assert False
	assert mean != 0.0 # keep this here; we can't proceed if mean = 0
	return 0.0
matrix_mean_nReads = get_mean_nReads()
...
def get_normalized_feats(base_feats: BaseFeats) -> NormalizedBaseFeats
...
def get_reader(feature: Any) -> Any:
	return csv.reader(feature, delimiter="\t")
...
if __name__ == "__main__":
	...
	with gzip.open(depth_feature_gz_path, "rt") as depth_feature, \
		...
		for depth_line, nReads_line, read_line, rfs_line in features_iterator:
			...
			normalized_base_feats: NormalizedBaseFeats = get_normalized_feats(base_feats)
